...
def print_cli_summary():
    ...
    if strat_a:
        print("\n" + "🏆 " + "—"*35 + " MODO A: APUESTAS SIMPLES DE VALOR (WIN RATE: 75%) " + "—"*35)
        print(f"   ► Enfoque: 3 Apuestas individuales independientes | Cuota Media: {strat_a.get('avgOdds')}x | EV+: {strat_a.get('combinedEv')}")
    ...
    if strat_b:
        ...
        print(f"   ► Modalidad: Sistema Trixie (3 Dobles + 1 Triple) | Éxito/Recuperación: {strat_b.get('expectedWinRate')} | EV+: {strat_b.get('combinedEv')}")
    ...
    if strat_c:
        print("\n" + "⚡ " + "—"*35 + " MODO C: DOBLE BANKER (DUPLICADOR @ 2.18x) " + "—"*35)
        print(f"   ► Cuota Total: {strat_c.get('totalOdds')}x | Confianza: {strat_c.get('expectedWinRate')} | EV+: {strat_c.get('combinedEv')}")
